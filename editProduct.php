...
if (isset($_GET["prodCode"])) {
    ...
    $query = $conn->query("SELECT DISTINCT Catagory FROM product");
} else {
...
if ($_SESSION['AccessLevel'] == 1) {
    ?>
<form action="editProduct.php?prodCode=<?= $prodCode ?>" method="post" enctype="multipart/form-data">
    ...
    if (in_array($fileActualExtension, $allowedExtensions)) {
        if ($fileError === 0) {
            ...
            if ($fileSize < 10000000000) {
                ...
                $sql = "UPDATE product SET ProductName= :newProdName, Catagory= :newProdCategory, Quantity= :newProdQuantity, productPrice= :newProdPrice, Image= :newProdImage, Code= :newProdCode WHERE code='$prodCode'";
